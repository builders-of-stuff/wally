<script lang="ts">
  ...
  import { postsStore } from '$lib/posts/posts.store.svelte.js';
  ...
  onMount(async () => {
    ...
    try {
      const fetchedPost = await postsStore.loadPost(postId);
      if (fetchedPost) {
        post = fetchedPost;
      } else {
        notFound = true;
  ...
  async function handleDelete() {
    ...
    try {
      const success = await postsStore.deletePost(post.id);
      if (success) {
        goto('/');
      } else {
